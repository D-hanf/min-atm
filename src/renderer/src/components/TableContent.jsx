  ...
  return (
    <>
      <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md overflow-hidden w-full`}>
        {/* Header */}
        <div className={`p-4 border-b flex flex-wrap items-center justify-between gap-4 ${isDark ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
          ...
          <div className="flex flex-wrap gap-3 w-full sm:w-auto items-center justify-end">
            {showDateFilter && (
              <div className="flex flex-col">
                <label className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Filter Tanggal</label>
                <input
                  type="date"
                  value={selectedDate}
        ...
        <div className="overflow-x-auto">
          <table className={`min-w-full divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
            <thead className={isDark ? 'bg-gray-700' : 'bg-gray-50'}>
              <tr>
                <th className={`px-6 py-3 text-left text-xs font-medium ${isDark ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider w-16`}>No</th>
                {columns.map((col) => (
                  <th key={col.key} className={`px-6 py-3 text-left text-xs font-medium ${isDark ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>{col.label}</th>
                ))}
                <th className={`px-6 py-3 text-right text-xs font-medium uppercase tracking-wider ${isDark ? 'text-gray-300 bg-gray-700' : 'text-gray-500 bg-gray-50'} sticky right-0 z-10`}>Aksi</th>
              </tr>
            </thead>
            <tbody className={`${isDark ? 'bg-gray-800 divide-y divide-gray-700' : 'bg-white divide-y divide-gray-200'}`}>
              {currentData.map((item, index) => (
                <tr key={item.id ?? index} className={isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                  ...
                  {columns.map((col) => (
                    <td key={col.key} className="px-6 py-4 text-sm whitespace-nowrap truncate max-w-[180px]">
                  ...
                  <td className={`px-6 py-4 text-right text-sm font-medium sticky right-0 z-10 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                    <div className="flex justify-end gap-2">
                      ...
                      <ButtonInput color="yellow" size={btnSize} onClick={() => handleEdit(item.id)}>
